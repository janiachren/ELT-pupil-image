...
Usage:
1. Copy the following files to the same directory: <code>run_regression.py</code> (test main), <code>ELTpupil_sttusReporting.py</code> (python main), <code>anisocado_pupUtils.py</code> (python libs), <code>elt_pupil_status</code> (C executable). Xml file not needed.
...
4. Script creates N xml files to a temporary directory (may be deleted after the test), creates FITS files with N_0 and compares them, then deletes FITS files, continues same with N_1, and repeats to N.
