...
/* ================================================================
   AUTO‑FILENAME GENERATOR (ESO‑STYLE)
   ================================================================ */

static void build_output_filename(char *out, size_t outlen)
{
    ...
    snprintf(out, outlen,
             "c.ELT.PUPIL.STATUS.%04d%02d%02dT%02d%02d%02d.fits",
             tm.tm_year + 1900,
             tm.tm_mon + 1,
             tm.tm_mday,
             tm.tm_hour,
...
static xmlNode* find_segments_node(xmlNode* root) {
    for (xmlNode* cur = root; cur; cur = cur->next) {
        if (cur->type == XML_ELEMENT_NODE &&
            ...
            return cur;
        xmlNode* found = find_segments_node(cur->children);
...
static int load_segments_from_file(const char* filename, double* F1, int n_segments) {
    ...
    if (!segments) {
        cpl_msg_error(__func__, "<segments> element not found.");
        xmlFreeDoc(doc);
        return 0;
    }
...
/* ================================================================
   MJD + CHECKSUM/DATASUM HELPERS
   ================================================================ */

static double compute_mjd(void)
{
    time_t now = time(NULL);
    return (double)now / 86400.0 + 40587.0;
}

static void compute_iso8601(char *buf, size_t len)
{
    time_t now = time(NULL);
    struct tm tm;
    localtime_r(&now, &tm);
...
/* FITS checksum algorithm (ESO-compatible) */
static unsigned long fits_checksum(const unsigned char *buf, size_t nbytes)
{
    unsigned long sum = 0;
...
static void encode_fits_checksum(char *out, unsigned long sum)
{
    /* FITS standard: ASCII-encoded complement */
    unsigned long complement = 0xFFFFFFFFUL - sum + 1;
    snprintf(out, 32, "%lu", complement);
}


/* ================================================================
   STANDALONE MAIN (CPL 7.x TOOL)
   ================================================================ */
...
int main(int argc, char *argv[])
{
    ...
    /* Generate ESO‑style output filename */
    char out_fits[256];
    build_output_filename(out_fits, sizeof(out_fits));

    /* Initialize CPL */
    cpl_init(CPL_INIT_DEFAULT);
    cpl_msg_info(cpl_func, "MICADO ELT pupil status tool (CPL 7.x)");
    cpl_msg_info(cpl_func, "Reading XML: %s", xml_file);
    cpl_msg_info(cpl_func, "Auto‑generated output: %s", out_fits);

    /* Load segment reflectivities */
    double *F1 = cpl_malloc(NUM_SEGMENTS * sizeof(double));
    if (!F1) {
        cpl_msg_error(cpl_func, "Cannot allocate F1");
        cpl_end();
        return 1;
    }

    if (!load_segments_from_file(xml_file, F1, NUM_SEGMENTS)) {
        cpl_msg_error(cpl_func, "Failed to load segments from XML");
        cpl_free(F1);
        cpl_end();
        return 1;
    }

    /* Generate pupil reflectivity map */
    double *pupil = generateEeltPupilReflectivity(
    ...
    if (!pupil) {
        cpl_msg_error(cpl_func, "generateEeltPupilReflectivity failed");
        cpl_end();
        return 1;
    }

    /* Create CPL image and copy data */
    cpl_image *img = cpl_image_new(GRID_SIZE, GRID_SIZE, CPL_TYPE_DOUBLE);
    if (!img) {
        cpl_msg_error(cpl_func, "Cannot allocate CPL image");
        cpl_free(pupil);
        cpl_end();
        return 1;
    }
...
/*    cpl_free(pupil); */
    ...
    /* Standard ESO keywords */
    cpl_propertylist_update_string(hdr, "INSTRUME", "MICADO");
    ...
    /* === MJD + DATE-OBS === */
    double mjd = compute_mjd();
    cpl_propertylist_update_double(hdr, "MJD-OBS", mjd);

    char dateobs[64];
    compute_iso8601(dateobs, sizeof(dateobs));
    cpl_propertylist_update_string(hdr, "DATE-OBS", dateobs);

    /* === CHECKSUM + DATASUM === */
    size_t npix = (size_t)GRID_SIZE * GRID_SIZE;
    unsigned long datasum = fits_checksum((unsigned char*)pupil, npix * sizeof(double));

    char datasum_str[32];
    snprintf(datasum_str, sizeof(datasum_str), "%lu", datasum);
    cpl_propertylist_update_string(hdr, "DATASUM", datasum_str);

    char checksum_str[32];
    encode_fits_checksum(checksum_str, datasum);
    cpl_propertylist_update_string(hdr, "CHECKSUM", checksum_str);


    /* Save FITS */
    cpl_error_code err = cpl_image_save(img, out_fits,
                                        CPL_BPP_IEEE_DOUBLE,
                                        hdr, CPL_IO_DEFAULT);

    if (err != CPL_ERROR_NONE) {
        cpl_msg_error(cpl_func, "Failed to save FITS: %s",
    ...
    cpl_msg_info(cpl_func, "Wrote pupil image to %s", out_fits);
    cpl_free(pupil);
    cpl_propertylist_delete(hdr);
    cpl_image_delete(img);
    cpl_end();
